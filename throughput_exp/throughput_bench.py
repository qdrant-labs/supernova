...
Usage:
  ...
  python throughput_bench.py --dataset HuggingfaceTB/dclm-edu --cutoffs 256,512,1024,2048 --batch-sizes 32 --sample 10000 --output results.json --hf-config None --model Qwen/Qwen3-Embedding-4B
...
def sample_texts(
    dataset: str, hf_config: str | None, split: str, column: str, n: int
) -> list[str]:
    """
    Stream and randomly sample n texts from a HuggingFace dataset.
    """
    print(
        f"Sampling {n:,} rows from {dataset} (config={hf_config}, column={column})..."
    )
...
def main():
    ...
    parser.add_argument(
        "--hf-config",
        default="en",
        help="HuggingFace dataset config (use 'None' to skip)",
    )
    ...
    parser.add_argument(
        "--dtypes",
        default="bfloat16",
        help="Comma-separated dtypes to test (float32,float16,bfloat16)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Truncate texts to this many chars before embedding",
    )
    parser.add_argument(
        "--cutoffs",
        default=None,
        help="Comma-separated token cutoffs to sweep (e.g. 256,512,1024,2048,4096)",
    )
    ...
    cutoffs_to_test = (
        [int(x) for x in args.cutoffs.split(",")] if args.cutoffs else [None]
    )
    dtype_map = {
    ...
    if args.max_text_length:
        ...
        print(
            f"Truncated texts: {before:,} -> {after:,} total chars ({100 * after / before:.1f}% kept)"
        )
    ...
    for dtype_name in dtypes_to_test:
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Loading model: {args.model} (dtype={dtype_name}, flash_attn={args.flash_attn})"
        )
        print(f"{'=' * 60}")
        ...
        for cutoff in cutoffs_to_test:
            ...
            if cutoff:
                trial_texts = [
                    tokenizer.decode(toks[:actual_cutoff], skip_special_tokens=True)
                    if len(toks) > actual_cutoff
                    else texts[i]
            ...
            for bs in batch_sizes:
                ...
                result.update(
                    {
                        ...
                        "tokenize_tok_per_s": round(
                            sum(len(t) for t in all_token_ids) / tokenize_time, 0
                        ),
                    }
                )
                ...
                print(
                    f"  {result['tokens_per_s']:,.0f} tok/s | {result['texts_per_s']:,.0f} texts/s | "
    ...
    if has_cutoffs:
        print(f"\n\n{'=' * 90}")
        print(
            f"{'Model':<40} {'dtype':<10} {'cutoff':>7} {'batch':>6} {'tok/s':>10} {'texts/s':>10} {'VRAM GB':>8}"
        )
        print(f"{'=' * 90}")
        for r in all_results:
            co = f"{r['cutoff']:,}" if r["cutoff"] else "none"
            print(
                f"{r['model']:<40} {r['dtype']:<10} {co:>7} {r['batch_size']:>6} "
                f"{r['tokens_per_s']:>10,.0f} {r['texts_per_s']:>10,.0f} {r['gpu_mem_peak_gb']:>8.1f}"
            )
    else:
        print(f"\n\n{'=' * 80}")
        print(
            f"{'Model':<40} {'dtype':<10} {'batch':>6} {'tok/s':>10} {'texts/s':>10} {'VRAM GB':>8}"
        )
        print(f"{'=' * 80}")
        for r in all_results:
            print(
                f"{r['model']:<40} {r['dtype']:<10} {r['batch_size']:>6} "
                f"{r['tokens_per_s']:>10,.0f} {r['texts_per_s']:>10,.0f} {r['gpu_mem_peak_gb']:>8.1f}"
            )
