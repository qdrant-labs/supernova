...
///
/// A free function, not a `From` impl: both `VectorValue` (the shared contract
/// crate) and `Vector` (qdrant-client) are now foreign to this crate, so the
/// orphan rule forbids `impl From<VectorValue> for Vector` here.
fn vector_value_to_wire(value: VectorValue) -> Vector {
    match value {
...
/// A read [`Point`] → a qdrant `PointStruct`. Also a free function (not `From`)
/// for the same orphan-rule reason as [`vector_value_to_wire`].
fn point_to_wire(point: Point) -> PointStruct {
    let vectors: HashMap<String, Vector> = point
        .vectors
        .into_iter()
        .map(|(name, value)| (name, vector_value_to_wire(value)))
        .collect();
...
impl VectorStore for QdrantStore {
    ...
    async fn upsert_batch(&self, points: Vec<Point>) -> Result<(), StoreError> {
        let points: Vec<PointStruct> = points.into_iter().map(point_to_wire).collect();
        self.client
            .upsert_points(
