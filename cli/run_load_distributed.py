...
def load_dist(config, dry_run, num_shards, pool_name, on_demand, ramp, finalize):
    ...
    pool_name_eff = pool_name or f"vf-load-{run_name}"
    ...
    click.echo("=" * 60)
    click.echo("vectorforge distributed loading plan")
    click.echo("=" * 60)
    ...
    job_yaml = {
        ...
        "resources": resources,
        "run": f"cd /app && uv run vf load {config} --num-jobs {num_shards_eff} --no-manage-indexing",
    }
    ...
    click.echo(f"  vf load-dist {config} --finalize")
