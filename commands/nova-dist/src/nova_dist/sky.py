...
def _rust_worker_setup(*binaries: str) -> str:
    """
    Install one or more prebuilt Rust binaries on a worker (seconds each),
    falling back to a source compile if no release asset matches the arch (e.g.
    before the first release). Installs to `/usr/local/bin` so they're on `PATH`
    in both SkyPilot's `setup` and `run` shells (they're separate).

    A `nova <cmd>` is now a thin shim (`nova-<cmd>`) that execs a backend
    executable (`nova-<cmd>-<backend>`), so a worker needs BOTH — pass both
    (e.g. `nova-load` and `nova-load-qdrant`).
    """
    header = (
        "set -e\n"
        'case "$(uname -m)" in\n'
...
DEFAULTS: dict[str, dict] = {
    "embed": {
        ...
        "setup": _python_worker_setup(
            "nova-embed",
            f"nova-embed[embed] @ git+{_REPO}@master#subdirectory=commands/nova-embed",
        ),
        "envs": {"HF_HUB_ENABLE_HF_TRANSFER": "1"},
    },
    "load": {
        "resources": {"cloud": "aws", "cpus": "8+", "use_spot": False, "disk_size": 100},
        "setup": _rust_worker_setup("nova-load", "nova-load-qdrant"),
    },
    "storm": {
        "resources": {"cloud": "aws", "cpus": "4+", "use_spot": False},
        "setup": _rust_worker_setup("nova-storm", "nova-storm-qdrant"),
    },
    "bf": {
        ...
        "setup": _python_worker_setup(
            "nova-bf",
            f"nova-bf[compute] @ git+{_REPO}@master#subdirectory=commands/nova-bf",
        ),
    },
}
