...
def test_collection_name_comes_from_the_config_plus_layout_name():
    cfg = _cfg(data_layouts={"vectors.dense.datatype": ["uint8"]})
    slices = build_slices(cfg)
    assert slices[0].collection_name == "mycollection_datatypeuint8"
...
def test_no_axes_declared_yields_one_slice_named_exactly_as_configured():
    cfg = _cfg()
    slices = build_slices(cfg)
    assert len(slices) == 1
    assert slices[0].collection_name == "mycollection"
