#!/usr/bin/env python3
"""
Dispatch distributed `nova partition` jobs via SkyPilot pools.

Same shape as `nova embed-dist` but with CPU-only resources (no GPU) and the
...
@click.option(
    "--on-demand", is_flag=True, help="Use on-demand instances instead of spot."
)
@click.option(
    "--ramp",
    is_flag=True,
...
def partition_dist(
    ...
    """Dispatch distributed `nova partition` via SkyPilot pools."""
    logging.basicConfig(
    ...
    resources = (
        partition_cfg.get("resources")
        or cfg.get("resources")
        or dict(DEFAULT_RESOURCES)
    )
    if on_demand:
    ...
    pool_name_eff = pool_name or f"nova-partition-{config_name}"
    ...
    click.echo("=" * 60)
    click.echo("supernova distributed partition plan (no-op embedder)")
    click.echo("=" * 60)
    ...
    click.echo(
        "  uv run python scripts/verify_no_duplicates.py s3://<your-prefix>/ --content-columns text"
    )
