...
//#endregion
//#region node_modules/umap-js/dist/utils.js
var require_utils = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function empty(n) {
		var output = [];
		for (var i = 0; i < n; i++) output.push(void 0);
		return output;
	}
	exports.empty = empty;
	function range(n) {
	...
	function max(input) {
		var max = 0;
		for (var i = 0; i < input.length; i++) max = input[i] > max ? input[i] : max;
		return max;
	}
	exports.max = max;
	function max2d(input) {
		var max = 0;
		for (var i = 0; i < input.length; i++) for (var j = 0; j < input[i].length; j++) max = input[i][j] > max ? input[i][j] : max;
		return max;
	}
	exports.max2d = max2d;
	function rejectionSample(nSamples, poolSize, random) {
		...
		for (var i = 0; i < nSamples; i++) {
			...
			while (rejectSample) {
				...
				var broken = false;
				for (var k = 0; k < i; k++) if (j === result[k]) {
					broken = true;
					break;
				}
	...
	function reshape2d(x, a, b) {
		...
		var count = 0;
		var index = 0;
		if (x.length !== a * b) throw new Error("Array dimensions must match input length.");
		for (var i = 0; i < a; i++) {
			var col = [];
			for (var j = 0; j < b; j++) {
				col.push(x[index]);
...
//#endregion
//#region node_modules/umap-js/dist/heap.js
var require_heap = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function uncheckedHeapPush(heap, row, weight, index, flag) {
		...
		while (true) {
			...
			else if (ic2 >= heapShape2) if (weights[ic1] > weight) iSwap = ic1;
			else break;
			else if (weights[ic1] >= weights[ic2]) if (weight < weights[ic1]) iSwap = ic1;
			else break;
			else if (weight < weights[ic2]) iSwap = ic2;
			else break;
	...
	function buildCandidates(currentGraph, nVertices, nNeighbors, maxCandidates, random) {
		...
		for (var i = 0; i < nVertices; i++) for (var j = 0; j < nNeighbors; j++) {
			if (currentGraph[0][i][j] < 0) continue;
	...
	function siftDown(heap1, heap2, ceiling, elt) {
		while (elt * 2 + 1 < ceiling) {
			...
			var swap = elt;
			if (heap1[swap] < heap1[leftChild]) swap = leftChild;
			if (rightChild < ceiling && heap1[swap] < heap1[rightChild]) swap = rightChild;
			if (swap === elt) break;
			else {
				var temp1 = heap1[elt];
	...
	function smallestFlagged(heap, row) {
		...
		var resultIndex = -1;
		for (var i = 0; i > ind.length; i++) if (flag[i] === 1 && dist[i] < minDist) {
			minDist = dist[i];
			resultIndex = i;
		}
		if (resultIndex >= 0) {
...
//#endregion
//#region node_modules/umap-js/dist/matrix.js
var require_matrix$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o, m, k, k2) {
		if (k2 === void 0) k2 = k;
		Object.defineProperty(o, k2, {
			enumerable: true,
	...
	var SparseMatrix = function() {
		function SparseMatrix(rows, cols, values, dims) {
			...
			this.nRows = 0;
			this.nCols = 0;
			if (rows.length !== cols.length || rows.length !== values.length) throw new Error("rows, cols and values arrays must all have the same length");
			this.nRows = dims[0];
			this.nCols = dims[1];
		...
		SparseMatrix.prototype.checkDims = function(row, col) {
			if (!(row < this.nRows && col < this.nCols)) throw new Error("row and/or col specified outside of matrix dimensions");
		};
		SparseMatrix.prototype.set = function(row, col, value) {
			...
			else this.entries.get(key).value = value;
		};
		...
		SparseMatrix.prototype.getAll = function(ordered) {
			...
			if (ordered) rowColValues.sort(function(a, b) {
		...
		SparseMatrix.prototype.toArray = function() {
			var _this = this;
			var output = utils.empty(this.nRows).map(function() {
	...
	function transpose(matrix) {
		...
		return new SparseMatrix(rows, cols, vals, [matrix.nCols, matrix.nRows]);
	}
	exports.transpose = transpose;
	function identity(size) {
		var rows = __read(size, 1)[0];
		var matrix = new SparseMatrix([], [], [], size);
		for (var i = 0; i < rows; i++) matrix.set(i, i, 1);
		return matrix;
	}
	...
	function eliminateZeros(m) {
		...
		var rows = m.getRows();
		var cols = m.getCols();
		for (var i = 0; i < values.length; i++) if (values[i] === 0) zeroIndices.add(i);
		var removeByZeroIndex = function(_, index) {
			return !zeroIndices.has(index);
		};
		var nextValues = values.filter(removeByZeroIndex);
		return new SparseMatrix(rows.filter(removeByZeroIndex), cols.filter(removeByZeroIndex), nextValues, m.getDims());
	}
	...
	function normalize(m, normType) {
		...
		var _loop_1 = function(row) {
			...
			var norm = normFn(cols.map(function(col) {
				return m.get(row, col);
			}));
			for (var i = 0; i < norm.length; i++) nextMatrix.set(row, cols[i], norm[i]);
		};
		try {
	...
	var normFns = (_a = {}, _a["max"] = function(xs) {
		var max = -Infinity;
		for (var i = 0; i < xs.length; i++) max = xs[i] > max ? xs[i] : max;
		...
		var sum = 0;
		for (var i = 0; i < xs.length; i++) sum += Math.pow(xs[i], 2);
		return xs.map(function(x) {
	...
	function elementWise(a, b, op) {
		...
		return new SparseMatrix(rows, cols, vals, [a.nRows, a.nCols]);
	}
	function getCSR(x) {
		var entries = [];
...
//#endregion
//#region node_modules/umap-js/dist/tree.js
var require_tree = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function makeForest(data, nNeighbors, nTrees, random) {
		...
		return utils.range(nTrees).map(function(_, i) {
	...
	function makeTree(data, leafSize, n, random) {
		...
		return makeEuclideanTree(data, utils.range(data.length), leafSize, n, random);
	}
	function makeEuclideanTree(data, indices, leafSize, q, random) {
		...
		if (indices.length > leafSize) {
			...
			var node = {
				leftChild: makeEuclideanTree(data, indicesLeft, leafSize, q + 1, random),
				rightChild: makeEuclideanTree(data, indicesRight, leafSize, q + 1, random),
				isLeaf: false,
				hyperplane,
				offset
			};
	...
	function euclideanRandomProjectionSplit(data, indices, random) {
		...
		for (var i = 0; i < indices.length; i++) {
			...
			for (var d = 0; d < dim; d++) margin += hyperplaneVector[d] * data[indices[i]][d];
			if (margin === 0) {
		...
		nLeft = 0;
		nRight = 0;
		for (var i = 0; i < side.length; i++) if (side[i] === 0) {
	...
	function numNodes(tree) {
		...
		else return 1 + numNodes(tree.leftChild) + numNodes(tree.rightChild);
	}
	function numLeaves(tree) {
		if (tree.isLeaf) return 1;
		else return numLeaves(tree.leftChild) + numLeaves(tree.rightChild);
	}
	...
	function selectSide(hyperplane, offset, point, random) {
		var margin = offset;
		for (var d = 0; d < point.length; d++) margin += hyperplane[d] * point[d];
		if (margin === 0) return utils.tauRandInt(2, random);
		else if (margin > 0) return 0;
		else return 1;
	}
	function searchFlatTree(point, tree, random) {
		var node = 0;
		while (tree.children[node][0] > 0) if (selectSide(tree.hyperplanes[node], tree.offsets[node], point, random) === 0) node = tree.children[node][0];
		else node = tree.children[node][1];
...
//#endregion
//#region node_modules/umap-js/dist/nn_descent.js
var require_nn_descent = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o, m, k, k2) {
		if (k2 === void 0) k2 = k;
		Object.defineProperty(o, k2, {
			enumerable: true,
	...
	var matrix = __importStar(require_matrix$1());
	...
	function makeNNDescent(distanceFn, random) {
		return function nNDescent(data, leafArray, nNeighbors, nIters, maxCandidates, delta, rho, rpTreeInit) {
			...
			if (rpTreeInit) for (var n = 0; n < leafArray.length; n++) for (var i = 0; i < leafArray[n].length; i++) {
				if (leafArray[n][i] < 0) break;
				for (var j = i + 1; j < leafArray[n].length; j++) {
					if (leafArray[n][j] < 0) break;
			...
			for (var n = 0; n < nIters; n++) {
				...
				var c = 0;
				for (var i = 0; i < nVertices; i++) for (var j = 0; j < maxCandidates; j++) {
					var p = Math.floor(candidateNeighbors[0][i][j]);
					if (p < 0 || utils.tauRand(random) < rho) continue;
				...
				while (true) {
					...
					try {
						for (var candidates_1 = (e_1 = void 0, __values(candidates)), candidates_1_1 = candidates_1.next(); !candidates_1_1.done; candidates_1_1 = candidates_1.next()) {
							...
							if (candidate === vertex || candidate === -1 || tried.has(candidate)) continue;
...
//#endregion
//#region node_modules/is-any-array/src/index.js
...
//#endregion
//#region node_modules/ml-levenberg-marquardt/src/errorCalculation.js
/**
...
function errorCalculation(data, parameters, parameterizedFunction) {
	...
	for (let i = 0; i < data.x.length; i++) error += Math.abs(data.y[i] - func(data.x[i]));
	return error;
}
var init_errorCalculation = __esmMin((() => {}));
//#endregion
//#region node_modules/ml-matrix/matrix.js
var require_matrix = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	var AbstractMatrix = class AbstractMatrix {
		...
		gram() {
			...
			for (let r = 0; r < rows; r++) for (let i = 0; i < n; i++) {
				...
				if (value === 0) continue;
				const offset = i * n;
				for (let j = i; j < n; j++) gramData[offset + j] += value * this.get(r, j);
			}
		...
		transposeMultiply(other) {
			other = Matrix.checkMatrix(other);
			if (this.rows !== other.rows) throw new RangeError("the number of rows of the two matrices must be equal");
			const n = this.columns;
			const p = other.columns;
			const result = new Matrix(n, p);
			const otherRow = new Float64Array(p);
			for (let r = 0; r < this.rows; r++) {
				for (let j = 0; j < p; j++) otherRow[j] = other.get(r, j);
				for (let i = 0; i < n; i++) {
					const value = this.get(r, i);
					if (value === 0) continue;
					const resultRow = result.data[i];
					for (let j = 0; j < p; j++) resultRow[j] += value * otherRow[j];
				}
			}
			return result;
		}
		mmulByTranspose(scale) {
			let m = this.rows;
			let n = this.columns;
			if (scale !== void 0 && scale.length !== n) throw new RangeError("scale must have one value per column");
			let result = new Matrix(m, m);
			let rowj = new Float64Array(n);
			for (let j = 0; j < m; j++) {
				if (scale === void 0) for (let k = 0; k < n; k++) rowj[k] = this.get(j, k);
				else for (let k = 0; k < n; k++) rowj[k] = scale[k] * this.get(j, k);
				for (let i = j; i < m; i++) {
					let s = 0;
					for (let k = 0; k < n; k++) s += this.get(i, k) * rowj[k];
					result.set(i, j, s);
					result.set(j, i, s);
				}
			}
	...
	var SingularValueDecomposition = class {
		constructor(value, options = {}) {
			...
			for (let k = 0; k < mrc; k++) {
				if (k < nct) {
					s[k] = 0;
					for (let i = k; i < m; i++) s[k] = hypotenuse(s[k], at.get(k, i));
					if (s[k] !== 0) {
						if (at.get(k, k) < 0) s[k] = -s[k];
						for (let i = k; i < m; i++) at.set(k, i, at.get(k, i) / s[k]);
				...
				for (let j = k + 1; j < n; j++) {
					if (k < nct && s[k] !== 0) {
						let t = 0;
						for (let i = k; i < m; i++) t += at.get(k, i) * at.get(j, i);
						t = -t / at.get(k, k);
						for (let i = k; i < m; i++) at.set(j, i, at.get(j, i) + t * at.get(k, i));
					}
					e[j] = at.get(j, k);
				}
				if (wantu && k < nct) for (let i = k; i < m; i++) U.set(k, i, at.get(k, i));
				if (k < nrt) {
					e[k] = 0;
					...
					if (k + 1 < m && e[k] !== 0) {
						...
						for (let i = k + 1; i < m; i++) for (let j = k + 1; j < n; j++) work[i] += e[j] * at.get(j, i);
						for (let j = k + 1; j < n; j++) {
							let t = -e[j] / e[k + 1];
							for (let i = k + 1; i < m; i++) at.set(j, i, at.get(j, i) + t * work[i]);
						}
					}
					if (wantv) for (let i = k + 1; i < n; i++) V.set(k, i, e[i]);
				}
			}
			let p = Math.min(n, m + 1);
			if (nct < n) s[nct] = at.get(nct, nct);
			if (m < p) s[p - 1] = 0;
			if (nrt + 1 < p) e[nrt] = at.get(p - 1, nrt);
			e[p - 1] = 0;
			if (wantu) {
				for (let j = nct; j < nu; j++) {
					for (let i = 0; i < m; i++) U.set(j, i, 0);
					U.set(j, j, 1);
				}
				for (let k = nct - 1; k >= 0; k--) if (s[k] !== 0) {
					for (let j = k + 1; j < nu; j++) {
						let t = 0;
						for (let i = k; i < m; i++) t += U.get(k, i) * U.get(j, i);
						t = -t / U.get(k, k);
						for (let i = k; i < m; i++) U.set(j, i, U.get(j, i) + t * U.get(k, i));
					}
					for (let i = k; i < m; i++) U.set(k, i, -U.get(k, i));
					U.set(k, k, 1 + U.get(k, k));
					for (let i = 0; i < k - 1; i++) U.set(k, i, 0);
				} else {
					for (let i = 0; i < m; i++) U.set(k, i, 0);
					U.set(k, k, 1);
				}
			}
			if (wantv) for (let k = n - 1; k >= 0; k--) {
				if (k < nrt && e[k] !== 0) for (let j = k + 1; j < n; j++) {
					let t = 0;
					for (let i = k + 1; i < n; i++) t += V.get(k, i) * V.get(j, i);
					t = -t / V.get(k, k + 1);
					for (let i = k + 1; i < n; i++) V.set(j, i, V.get(j, i) + t * V.get(k, i));
				}
				for (let i = 0; i < n; i++) V.set(k, i, 0);
				V.set(k, k, 1);
			}
			let pp = p - 1;
			...
			while (p > 0) {
				...
				switch (kase) {
					...
					case 4:
						if (s[k] <= 0) {
							s[k] = s[k] < 0 ? -s[k] : 0;
							if (wantv) for (let i = 0; i <= pp; i++) V.set(k, i, -V.get(k, i));
						}
						while (k < pp) {
							if (s[k] >= s[k + 1]) break;
							let t = s[k];
...
})), import_matrix, Matrix, inverse;
var init_matrix = __esmMin((() => {
	import_matrix = /* @__PURE__ */ __toESM(require_matrix(), 1);
	Matrix = import_matrix.Matrix;
	import_matrix.Matrix ? import_matrix.Matrix : import_matrix.Matrix;
	inverse = import_matrix.inverse;
}));
//#endregion
//#region node_modules/ml-levenberg-marquardt/src/step.js
/**
...
function gradientFunction(data, evaluatedData, params, gradientDifference, paramFunction) {
	...
	for (let param = 0; param < n; param++) {
		...
		for (let point = 0; point < m; point++) ans[param][point] = evaluatedData[point] - funcParam(data.x[point]);
	}
...
function matrixFunction(data, evaluatedData) {
	const m = data.x.length;
	let ans = new Array(m);
	for (let point = 0; point < m; point++) ans[point] = [data.y[point] - evaluatedData[point]];
	return new Matrix(ans);
}
/**
...
function step(data, params, damping, gradientDifference, parameterizedFunction) {
	...
	for (let i = 0; i < data.x.length; i++) evaluatedData[i] = func(data.x[i]);
...
//#endregion
//#region node_modules/ml-levenberg-marquardt/src/index.js
var src_exports = /* @__PURE__ */ __exportAll({ default: () => levenbergMarquardt });
/**
...
function levenbergMarquardt(data, parameterizedFunction, options = {}) {
	let { maxIterations = 100, gradientDifference = .1, damping = 0, errorTolerance = .01, minValues, maxValues, initialValues } = options;
	if (damping <= 0) throw new Error("The damping option must be a positive number");
	else if (!data.x || !data.y) throw new Error("The data parameter must have x and y elements");
	else if (!isAnyArray(data.x) || data.x.length < 2 || !isAnyArray(data.y) || data.y.length < 2) throw new Error("The data parameter elements must be an array with more than 2 points");
	else if (data.x.length !== data.y.length) throw new Error("The data parameter elements must have the same size");
	let parameters = initialValues || new Array(parameterizedFunction.length).fill(1);
	let parLen = parameters.length;
	maxValues = maxValues || new Array(parLen).fill(Number.MAX_SAFE_INTEGER);
	minValues = minValues || new Array(parLen).fill(Number.MIN_SAFE_INTEGER);
	if (maxValues.length !== minValues.length) throw new Error("minValues and maxValues must be the same size");
	if (!isAnyArray(parameters)) throw new Error("initialValues must be an array");
	...
	for (iteration = 0; iteration < maxIterations && !converged; iteration++) {
		...
		for (let k = 0; k < parLen; k++) parameters[k] = Math.min(Math.max(minValues[k], parameters[k]), maxValues[k]);
...
//#endregion
//#region node_modules/umap-js/dist/umap.js
var require_umap = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o, m, k, k2) {
		if (k2 === void 0) k2 = k;
		Object.defineProperty(o, k2, {
			enumerable: true,
	...
	var tree = __importStar(require_tree());
	var utils = __importStar(require_utils());
	var ml_levenberg_marquardt_1 = __importDefault((init_src(), __toCommonJS(src_exports)));
	var SMOOTH_K_TOLERANCE = 1e-5;
	var MIN_K_DIST_SCALE = .001;
	exports.UMAP = function() {
		function UMAP(params) {
			...
			var setParam = function(key) {
				if (params[key] !== void 0) _this[key] = params[key];
			};
			setParam("distanceFn");
		...
		UMAP.prototype.initializeFit = function(X) {
			if (X.length <= this.nNeighbors) throw new Error("Not enough data points (" + X.length + ") to create nNeighbors: " + this.nNeighbors + ".  Add more data points or adjust the configuration.");
			if (this.X === X && this.isInitialized) return this.getNEpochs();
			this.X = X;
		...
		UMAP.prototype.makeSearchGraph = function(X) {
			...
			for (var i = 0; i < knnIndices.length; i++) {
				...
				for (var j = 0; j < knn.length; j++) {
					...
					if (distance > 0) searchGraph.set(i, neighbor, distance);
				}
			}
		...
		UMAP.prototype.transform = function(toTransform) {
			var _this = this;
			var rawData = this.X;
			if (rawData === void 0 || rawData.length === 0) throw new Error("No data has been fit.");
			...
			var embedding = initTransform(utils.reshape2d(csrMatrix.indices, nPoints, this.nNeighbors), utils.reshape2d(csrMatrix.values, nPoints, this.nNeighbors), this.embedding);
		...
		UMAP.prototype.processGraphForSupervisedProjection = function() {
			...
			if (Y) {
				if (Y.length !== X.length) throw new Error("Length of X and y must be equal");
				if (this.targetMetric === "categorical") {
					var farDist = this.targetWeight < 1 ? 2.5 * (1 / (1 - this.targetWeight)) : 0xe8d4a51000;
		...
		UMAP.prototype.step = function() {
			...
			if (currentEpoch < this.getNEpochs()) this.optimizeLayoutStep(currentEpoch);
		...
		UMAP.prototype.nearestNeighbors = function(X) {
			...
			var round = function(n) {
				return n === .5 ? 0 : Math.round(n);
			};
			var nTrees = 5 + Math.floor(round(Math.pow(X.length, .5) / 20));
			var nIters = Math.max(5, Math.floor(Math.round(log2(X.length))));
			this.rpForest = tree.makeForest(X, nNeighbors, nTrees, this.random);
			var _b = metricNNDescent(X, tree.makeLeafArray(this.rpForest), nNeighbors, nIters);
			return {
				knnIndices: _b.indices,
				knnDistances: _b.weights
			};
		};
		UMAP.prototype.fuzzySimplicialSet = function(X, nNeighbors, setOpMixRatio) {
			if (setOpMixRatio === void 0) setOpMixRatio = 1;
		...
		UMAP.prototype.smoothKNNDistance = function(distances, k, localConnectivity, nIter, bandwidth) {
			if (localConnectivity === void 0) localConnectivity = 1;
			if (nIter === void 0) nIter = 64;
			if (bandwidth === void 0) bandwidth = 1;
			...
			for (var i = 0; i < distances.length; i++) {
				...
				if (nonZeroDists.length >= localConnectivity) {
					...
					if (index > 0) {
						...
						if (interpolation > SMOOTH_K_TOLERANCE) rho[i] += interpolation * (nonZeroDists[index] - nonZeroDists[index - 1]);
					} else rho[i] = interpolation * nonZeroDists[0];
				} else if (nonZeroDists.length > 0) rho[i] = utils.max(nonZeroDists);
				for (var n = 0; n < nIter; n++) {
					var psum = 0;
					for (var j = 1; j < distances[i].length; j++) {
						var d = distances[i][j] - rho[i];
						if (d > 0) psum += Math.exp(-(d / mid));
						else psum += 1;
					}
				...
				if (rho[i] > 0) {
					...
					if (result[i] < MIN_K_DIST_SCALE * meanIthDistances) result[i] = MIN_K_DIST_SCALE * meanIthDistances;
				} else {
					var meanDistances = utils.mean(distances.map(utils.mean));
					if (result[i] < MIN_K_DIST_SCALE * meanDistances) result[i] = MIN_K_DIST_SCALE * meanDistances;
				}
			}
			return {
				sigmas: result,
		...
		UMAP.prototype.computeMembershipStrengths = function(knnIndices, knnDistances, sigmas, rhos) {
			...
			for (var i = 0; i < nSamples; i++) for (var j = 0; j < nNeighbors; j++) {
				var val = 0;
				if (knnIndices[i][j] === -1) continue;
				if (knnIndices[i][j] === i) val = 0;
				else if (knnDistances[i][j] - rhos[i] <= 0) val = 1;
				else val = Math.exp(-((knnDistances[i][j] - rhos[i]) / sigmas[i]));
		...
		UMAP.prototype.initializeSimplicialSetEmbedding = function() {
			...
			for (var i = 0; i < graphValues.length; i++) {
				...
				if (graphMax < graphValues[i]) graphMax = value;
			}
			var graph = this.graph.map(function(value) {
				if (value < graphMax / nEpochs) return 0;
				else return value;
			});
			...
			return {
				head,
				tail,
				epochsPerSample: this.makeEpochsPerSample(weights, nEpochs)
			};
		};
		...
		UMAP.prototype.optimizeLayoutStep = function(n) {
			...
			for (var i = 0; i < epochsPerSample.length; i++) {
				...
				for (var d = 0; d < dim; d++) {
					...
					if (moveOther) other[d] += -gradD * alpha;
				}
				...
				for (var p = 0; p < nNegSamples; p++) {
					...
					if (distSquared_1 > 0) {
						...
						gradCoeff_1 /= (.001 + distSquared_1) * (a * Math.pow(distSquared_1, b) + 1);
					} else if (j === k_1) continue;
					for (var d = 0; d < dim; d++) {
						var gradD = 4;
						if (gradCoeff_1 > 0) gradD = clip(gradCoeff_1 * (current[d] - other_1[d]), clipValue);
						current[d] += gradD * alpha;
					}
				}
		...
		UMAP.prototype.optimizeLayoutAsync = function(epochCallback) {
			var _this = this;
			if (epochCallback === void 0) epochCallback = function() {
				return true;
			};
					...
					return __awaiter(_this, void 0, void 0, function() {
						...
						return __generator(this, function(_b) {
							try {
								...
								if (!shouldStop && !isFinished) setTimeout(function() {
									return step();
								}, 0);
								else return [2, resolve(isFinished)];
							} catch (err) {
								reject(err);
							}
							return [2];
		...
		UMAP.prototype.getNEpochs = function() {
			...
			if (length <= 2500) return 500;
			else if (length <= 5e3) return 400;
			else if (length <= 7500) return 300;
			else return 200;
		};
		return UMAP;
	}();
	function euclidean(x, y) {
		var result = 0;
		for (var i = 0; i < x.length; i++) result += Math.pow(x[i] - y[i], 2);
		return Math.sqrt(result);
	}
	...
	function cosine(x, y) {
		...
		else if (normX === 0 || normY === 0) return 1;
		else return 1 - result / Math.sqrt(normX * normY);
	}
	...
	function findABParams(spread, minDist) {
		...
		var yv = utils.zeros(xv.length).map(function(val, index) {
			return xv[index] >= minDist ? Math.exp(-(xv[index] - minDist) / spread) : val;
		});
		var initialValues = [.5, .5];
		var data = {
			x: xv,
			y: yv
		};
	...
	function fastIntersection(graph, target, unknownDist, farDist) {
		if (unknownDist === void 0) unknownDist = 1;
		if (farDist === void 0) farDist = 5;
		return graph.map(function(value, row, col) {
			if (target[row] === -1 || target[col] === -1) return value * Math.exp(-unknownDist);
			else if (target[row] !== target[col]) return value * Math.exp(-farDist);
			else return value;
		});
	}
	...
	function initTransform(indices, weights, embedding) {
		...
		for (var i = 0; i < indices.length; i++) for (var j = 0; j < indices[0].length; j++) for (var d = 0; d < embedding[0].length; d++) {
			var a = indices[i][j];
...
//#endregion
//#region node_modules/umap-js/dist/index.js
