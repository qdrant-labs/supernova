...
//!
//! Like `nova-load`, each layer has its own enum and backend-specific variants
//! are gated behind the backend's feature so the core never names a client
//! crate that isn't compiled in. [`StormError`] aggregates them for the binary.
