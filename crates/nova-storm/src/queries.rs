...
//! (a local path or an `s3://` URI), read once at startup via DuckDB. The set
//! is held in memory and reused round-robin across the whole run, so this is a
//! one-shot synchronous read — there's nothing to stream or overlap.
...
pub fn load_query_vectors(source: &QuerySource) -> Result<Vec<Vec<f32>>, QueryLoadError> {
    ...
    // Config is trusted (operator-authored), matching the Python loader's plain
    // interpolation. The column is quoted so names with odd characters survive.
    let sql = format!(
...
/// Mirrors `nova-load`'s S3 backend so query parquets and corpus parquets read
