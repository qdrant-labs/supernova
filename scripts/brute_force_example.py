...
sample_embeddings = pl.read_parquet("/Users/nathanleroy/Downloads/batch_00000000.parquet")
...
for row in tqdm(queries.iter_rows(), total=queries.shape[0]):
    ...
    similarity_scores = np.dot(sample_embeddings_array, query_embedding) / (np.linalg.norm(sample_embeddings_array, axis=1) * np.linalg.norm(query_embedding) + 1e-10)
    sample_embeddings = sample_embeddings.with_columns(pl.Series("similarity", similarity_scores))
